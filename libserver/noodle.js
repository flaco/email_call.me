...
noodle.get = function (path, handler) {
  getMap[path] = handler;
};
var server = createServer(function (req, res) {
  if (req.method === "GET" || req.method === "HEAD") {
    ...
    res.simpleJSON = function (code, noodle) {
      var body = JSON.stringify(noodle);
...
noodle.listen = function (port, host) {
  server.listen(port, host);
  sys.puts("Server at http://" + (host || "127.0.0.1") + ":" + port.toString() + "/");
};

noodle.close = function () { server.close(); };
...
noodle.staticHandler = function (filename) {
  var body, headers;
  var content_type = noodle.mime.lookupExtension(extname(filename));
...
noodle.mime = {
  lookupExtension : function(ext, fallback) {
    return noodle.mime.TYPES[ext.toLowerCase()] || fallback || 'application/octet-stream';
  },
